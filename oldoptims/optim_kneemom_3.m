function [result] = optim_kneemom_3(prob);
...
% predictive optimization
	...
	ncon = nstates * N+N;						% number of constraints due to discretized dynamics and task
...
%      end
	...
	if (problem.MaxIterations > 0)	
		...
		if strcmp(problem.Solver,'IPOPT')
			...
			options.cu = [zeros(ncon-N,1);ones(N,1)*inf];	
...
function [f, g, c, J] = evaluate(model, X)
    ...
    temp = load('Winter/Winter_normal_result_bka_450');
    temp.model = initmodel(temp.result.model);
    for ii=1:size(temp.result.x,2)
        mom(:,ii) = temp.model.gait2d('Jointmoments',temp.result.x(1:50,ii));
    end
    RKneemom = mom(2,:);
	...
	for i=1:N
        ...
        % Constraint on joint moments: the moment in the right knee should be
        % smaller than or equal to the joint moment in the able-bodied solution 
        [M,dMdx] = jointmoments(x1, model);
        c(end-N+i) = RKneemom(i) - M;
        J(end-N+i, ix1) = dMdx;
...
function evaluate_if_needed(X)
	...
	if problem.print == 2 || ~isfield(optim,'X') || (min(X == optim.X) == 0)
		...
		normc = norm(optim.c(1:end-problem.N));
		row = [normc optim.f];
		row(find(row==0)) = NaN;
