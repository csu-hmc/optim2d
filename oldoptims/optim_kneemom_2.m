function [result] = optim_kneemom_2(prob);
...
function [f, g, c, J] = evaluate(model, X)
    ...
    temp = load('Winter/Winter_normal_result_able');
	...
	for i=1:N
        ...
        % greater than or equal to the joint moment in the able-bodied solution 
        [M,dMdx] = jointmoments(x1, model);
        c(end-N+i) = M - RKneemom(i);
        J(end-N+i, ix1) = -dMdx;
