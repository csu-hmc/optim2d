function report_torque(resultfile)
...
%     xstd = std(x_mat,[],3);
	...
	ycontact = zeros(4,N);
	for i=1:N
		[GRF(:,i), dGRFdx, tmp(:,i)] = model.gait2d('GRF',xmean(1:50,i));
	...
	simmom = [utot([1:3],:) utot([4:6],:)]';
    simutot = [utot([1:3],:) utot([4:6],:)]';
...
% 	close all				% closes all existing Figure windows
	...
	% Plot joint angles
	subplot(3,4,2)
	plotvar(simang(:,1), 0,length(dur));
	ylabel('Angle [deg]', 'Fontsize',8')
	title('Hip angle');
	subplot(3,4,3)
	plotvar(-simang(:,2), 0,length(dur));
	ylabel('Angle [deg]', 'Fontsize',8')
	title('Knee angle');
	subplot(3,4,4)
	plotvar(simang(:,3), 0,length(dur));
	ylabel('Angle [deg]', 'Fontsize',8')
	title('Ankle angle');
...
%============================================================================
function plotvar(sim,bsymm,Ncycles);
